...
func FetchKeysetFromSource(vns3 *cn.VNS3Client, source string, token string, waitTimeout int) (*cn.KeysetStatus, error) {
	...
	for elapsed < float64(waitTimeout) {
		vns3.Log.Info("Sending set keyset request check")
		...
		if err != nil {
			...
			if isUnavailable {
				...
				vns3.Log.Info("Keyset fetch: connection error. VNS3 is likely rebooting. Waiting for API.")
				...
				if isApiError {
					...
					if strings.Contains(message, "in progress") {
						vns3.Log.Info("Keyset fetch in progress")
						...
						vns3.Log.Info("Keyset already exists. Waiting for VNS3 reboot and keyset")
						...
						vns3.Log.Info("keyset waitfor api")
...
func InitControllerFromSource(vns3 *cn.VNS3Client, source string, password string, timeout int) (*cn.SnapshotFetchStatusResponse, error) {
	// Initialize this controller from the controller specified in source

	// Arguments:
	//     source {str} - host controller to synchronize from
	//     password {str} - password for source controller
	//     timeout {float} - timeout for waiting synchronizing to source controlleer (default: 1 min)
	// """

	ctx := context.Background()
	fetchSnapshotParams := cn.NewFetchSnapshotParamsRequest(source)
	fetchSnapshotParams.SetSource(source)
	fetchSnapshotParams.SetUser("api")
	fetchSnapshotParams.SetPassword(password)
	fetchSnapshotParams.SetTimeout(timeout)
	fetchSnapshotRequest := vns3.SnapshotsApi.PutFetchSnapshotRequest(ctx)
	fetchSnapshotRequest = fetchSnapshotRequest.FetchSnapshotRequest(*fetchSnapshotParams)

	// initial put fetch snapshot
	...
	if wait_err != nil {
		...
		return nil, fmt.Errorf("VNS3 is not available [host=%v] %v", host, wait_err)
	}

	return SnapshotFetchStatusResponse, nil
}
